...
function read_input_and_validate() {
    local prompt=$1
    local default=$2
...
function validate_pinata_api_key() {
    if [[ ! $1 =~ ^[a-zA-Z0-9]+$ ]]; then
...
function validate_pinata_secret() {
    if [[ ! $1 =~ ^[a-zA-Z0-9]+$ ]]; then
...
function validate_telegram_bot_token() {
...
function validate_telegram_url() {
...
function deploy_wallet_contract() {
